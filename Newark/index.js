...
swiperElemAnim.style.animation = 'slide 7s ease-in-out forwards';
swiperElemAnim.style.animationDelay = '1.5s';

// Функция, которая будет вызвана после завершения анимации
swiperElemAnim.addEventListener('animationend', () => {
    console.log('Анимация завершена. Инициализация свайпера.');

    swiperElemAnim.style.animation = 'slideBack 7s ease-in-out forwards';
    ...
    // Переинициализация свайпера для учета нового элемента
    swiper._element = document.querySelector('.people-gallery-absolute');
    swiper._itemWidth = swiper._element.children[0].offsetWidth;
    swiper._maxTranslate = 0; // Пересчитываем максимальный сдвиг если нужно
    swiper._applyTranslate(); // Применяем текущий сдвиг
...
class Swiper extends EventTarget {
    constructor(element, options = { tresholdX: 10 }) {
        ...
        console.log(`Ширина элемента: ${this._itemWidth}, Максимальный сдвиг: ${this._maxTranslate}`);

        this._element.addEventListener('touchstart', this._handleTouchStart.bind(this), false);
        this._element.addEventListener('touchmove', this._handleTouchMove.bind(this), false);
    ...
    _swipeLeft() {
        ...
        if (this._currentTranslate > this._maxTranslate) {
            ...
            console.log(`Текущий сдвиг после свайпа влево: ${this._currentTranslate}`);
        } else {
    ...
    _swipeRight() {
        ...
        if (this._currentTranslate < this._minTranslate) {
            ...
            this._applyTranslate();
            console.log(`Текущий сдвиг после свайпа вправо: ${this._currentTranslate}`);
        } else {
